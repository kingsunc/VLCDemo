...
BOOL CplayerGUIDlg::OnInitDialog()
{
	...
	CStatic* pCtrl = (CStatic*)GetDlgItem(IDC_SCREEN);
	CBitmap bmp;
	...
	InvalidateRect(rectWnd);	// 重绘Pic控件区域;

	
	//pCtrl->SetBitmap(::LoadBitmap(theApp.m_hInstance, IDB_WELCOME));
...
void CplayerGUIDlg::OnSize(UINT nType, int cx, int cy)
{
	int iLeft = 20, iTop = 50, iRight = 20, iBottom = 100;
	GetDlgItem(IDC_SCREEN)->MoveWindow(iLeft, iTop, cx - (iLeft + iRight), cy - (iTop + iBottom));

	int iWidth = 60;
	GetDlgItem(IDC_CURTIME)->MoveWindow(iLeft, cy - (iBottom - 10), 60, 20);
	GetDlgItem(IDC_PROGRESS)->MoveWindow(iLeft + 80, cy - (iBottom - 10), cx - (iRight + iWidth) - (iLeft + 80), 20);
	GetDlgItem(IDC_DURATION)->MoveWindow(cx - (iRight + iWidth), cy - (iBottom - 10), iWidth, 20);

	int iWidthBtn = 50;
	GetDlgItem(ID_START)->MoveWindow(iLeft, cy - 60, iWidthBtn, 50);
	GetDlgItem(ID_PAUSE)->MoveWindow(iLeft + (iWidthBtn + 10), cy - 60, iWidthBtn, 50);
	GetDlgItem(ID_STOP)->MoveWindow(iLeft + (iWidthBtn + 10)*2, cy - 60, iWidthBtn, 50);
	GetDlgItem(ID_INFO)->MoveWindow(iLeft + (iWidthBtn + 10)*3, cy - 60, iWidthBtn, 50);
	GetDlgItem(ID_ABOUT)->MoveWindow(cx-60, cy - 60, iWidthBtn, 50);

	CRect rectWnd(0, 0, cx, cy);
	InvalidateRect(rectWnd);	// 重绘窗口区域;
